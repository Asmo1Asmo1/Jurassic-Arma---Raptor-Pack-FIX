...
_snd = ["RaptorHitCar1", "RaptorHitCar2", "RaptorHitCar3"] select (floor (random 3));
...
if (_open) then
{
   ...
   } else
   {
      if (count (crew _car) > 0) then
      {
         ...
         _rdam = 1*rup_raptor_dam_AI;

         [_unit, _rdam, false, "", _Ddir, "RaptorHit1", _dino] call rup_fnc_deal_damage;
      };
   };
} else
{
